        ...
        if epoch % eval_freq == 0 or epoch == (args.epochs - 1):
            with torch.no_grad():
                ...
                for t in [0.7, 0.8, 0.9, 1.0]:
                    output = denoiser.sample(num_samples, model, temperature=t)
...
def train(train_queue, model, denoiser, cnn_optimizer, grad_scalar, global_step, warmup_iters, writer, logging):
    ...
    for step, x in enumerate(train_queue):
        ...
        prev_x, a_prev = denoiser.sample_noised(x, torch.randn_like(x), timestep)
        next_x, a_next = denoiser.sample_noised(x, torch.randn_like(x), timestep-1)
        _, trg_x = denoiser.denoise_step(next_x, timestep-1)
        ...
        if ((global_step + 1) % 100 == 0) or (global_step == 0):
            if ((global_step + 1) % 1000 == 0) or (global_step == 0):  # reduced frequency
                ...
                _, output_img = denoiser.denoise_step(prev_x, timestep, eps=pred_eps.sample())
